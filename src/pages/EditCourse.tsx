...
const EditCourse = () => {
  ...
  const [coursePeopleData, setCoursePeopleData] = useState(["Ismoil"]);
  const [course_learns, setCourseLearns] = useState(["ReactJs"]);
  const [course_people_count, setCoursePeopleCount] = useState("");
  ...
  const addUser = (e: any) => {
    ...
    if (course_people_count.length > 0) {
      //   data.push(course_people_count);
      setCoursePeopleData([...coursePeopleData, course_people_count]);
      setCoursePeopleCount("");
    } else {
  ...
  const sendForm = (e: any) => {
    ...
    const formData = {
      course_id: `${id ? id : null}`,
      course_name,
      course_description,
      course_category,
      course_people_count: coursePeopleData,
      course_price,
      course_learns,
      course_duration,
      course_video,
    ...
    fetch("https://shohsulton.uz/api/cources/update", {
      method: "PUT",
      headers: {
      ...
      .then((data) => {
        if (data.statusCode === 200) {
          alert(data.message);
          //   if (sections?.course_sections.length > 0) {
  ...
  useEffect(() => {
    fetch(`https://shohsulton.uz/api/cources/${id}`)
      ...
      .then((data: any) => {
        ...
        setCourseDuration(data.data.course_duration);
        setCourseVideo(data.data.course_video);
        setCourseLearns(data.data.course_learns);
        setCoursePeopleData(data.data.course_people_count);
      ...
      <form className="w-auto my-auto" onSubmit={sendForm}>
        ...
        <label htmlFor="course_people_count" className="w-100 mb-3">
          ...
          <input
            ...
            value={course_people_count}
            placeholder="Оценка пользователей"
            className="form-control rounded mb-3"
            onChange={(e) => setCoursePeopleCount(e.target.value)}
          />
          <button
            type="submit"
            onClick={addUser}
          ...
          <table className="table border align-middle mb-0 bg-white">
            ...
            <tbody>
              {coursePeopleData.length > 0 &&
                coursePeopleData.map((el: any, index: number) => {
                  return (
                    <tr key={el.id}>
                      <td>
                        <p
